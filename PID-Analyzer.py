...
class CSV_log:
    ...
    def plot_all_noise(self, traces, lims): #style='fancy' gives 2d hist for response
        ...
        for i, tr in enumerate(traces):
            ...
            pc0 = plt.pcolormesh(tr.noise_gyro['throt_axis'][:-1], tr.noise_gyro['freq_axis'][:-1], tr.noise_gyro['hist2d_sm']+1.,norm=colors.LogNorm(vmin=lims[0,0],vmax=lims[0,1]),cmap=cmap, shading='gouraud')
            ...
            pc1 = plt.pcolormesh(tr.noise_debug['throt_axis'][:-1],tr.noise_debug['freq_axis'][:-1], tr.noise_debug['hist2d_sm']+1., norm=colors.LogNorm(vmin=lims[1,0],vmax=lims[1,1]),cmap=cmap, shading='gouraud')
            ...
            if i<2:
                ...
                pc2 = plt.pcolormesh(tr.noise_d['throt_axis'][:-1], tr.noise_d['freq_axis'][:-1], tr.noise_d['hist2d_sm']+1., norm=colors.LogNorm(vmin=lims[2,0],vmax=lims[2,1]),cmap=cmap, shading='gouraud')
    ...
    def plot_all_resp(self, traces, style='ra'): # style='raw' for response vs. time in color plot
        ...
        for i, tr in enumerate(traces):
            ...
            if style =='raw':
                ...
                plt.pcolormesh(tr.avr_t, tr.time_resp, np.transpose(tr.spec_sm), vmin=0, vmax=2., shading='gouraud')
                ...
                plt.pcolormesh(tr.thr_response['throt_scale'][:-1], tr.time_resp, tr.thr_response['hist2d_norm'], vmin=0., vmax=2., shading='gouraud')
                plt.ylabel('response time in s')
                ax2.get_yaxis().set_label_coords(-0.1, 0.5)
